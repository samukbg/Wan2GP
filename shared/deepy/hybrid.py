...
def launch_gradio(demo, service, args, **kwargs):
    ...
    from shared.ephemeral_cleanup import EphemeralCleanupMiddleware
    ...
    middleware.append(Middleware(EphemeralCleanupMiddleware))
    prevent_lock = kwargs.pop("prevent_thread_lock", https_port is not None)
    res = demo.launch(**kwargs, app_kwargs={"lifespan": lifespan, "middleware": middleware}, ssl_certfile=cert if https_port is None else None, ssl_keyfile=key if https_port is None else None, ssl_verify=False if cert else True, prevent_thread_lock=prevent_lock)
    if https_port is not None:
        import uvicorn
