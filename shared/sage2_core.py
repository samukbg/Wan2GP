...
def sageattn(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        # Note: PyTorch SDPA does not support attn_mask in the same way SageAttention does (or at all in some versions)
        # but we try to pass it if provided.
        o = F.scaled_dot_product_attention(q, k, v, attn_mask=attn_mask, scale=scale, is_causal=is_causal)
        if tensor_layout == "NHD":
